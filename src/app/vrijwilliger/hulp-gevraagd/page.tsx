...
export default async function VrijwilligerHulpGevraagdPage() {
  ...
  const items = await prisma.hulpGevraagd.findMany({
    ...
    include: {
      _count: {
        select: { reacties: { where: { status: { not: { in: ["afgewezen", "geweigerd"] } } } } },
      },
      reacties: {
            ...
            {mijnaanmeldingen.map((r) => (
              <div key={r.id} className="flex items-center gap-3 px-4 py-3">
                ...
                <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full flex-shrink-0 ${
                  ...
                  r.status === "geweigerd" ? "bg-neutral-100 text-neutral-500" :
                  "bg-sky-100 text-sky-700"
                }`}>
                  {r.status === "bevestigd" ? "Bevestigd" : r.status === "afgewezen" ? "Afgewezen" : r.status === "geweigerd" ? "Niet geholpen" : "Aangemeld"}
                </span>
              </div>
            ))}
          </div>
